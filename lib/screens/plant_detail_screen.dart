...
class _PlantDetailScreenState extends State<PlantDetailScreen>
  ...
  Widget build(BuildContext context) {
    ...
    if (plant == null) {
      return Scaffold(
        appBar: AppBar(),
        body: const Center(child: Text('Pianta non trovata')),
      );
    }
    final photos = Hive.box(
      'photos',
    ).values.cast<PhotoEntry>().where((p) => p.plantId == plant.id).toList();
    ...
    return Scaffold(
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    ...
                    title: const Text('Aggiungi da galleria'),
      ...
      bottomNavigationBar: SuggestionBanner(
        messages: _suggest.suggestionsFor(plant, DateTime.now()),
      ),
    );
  }
}
...
class _InfoTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        _tile(
          'Registrata',
          plant.createdAt.toLocal().toString().substring(0, 16),
        ),
        const SizedBox(height: 12),
        FilledButton.icon(
          onPressed: onOpenGallery,
          icon: const Icon(Icons.photo_library),
          label: const Text('Apri galleria'),
        ),
      ],
    );
  }
  ...
  String _label(WorkStage s) {
    switch (s) {
      case WorkStage.nursery:
        return 'Nursery';
      case WorkStage.firstSetup:
        return 'Prima impostazione';
      case WorkStage.refinement:
        return 'Rifinitura';
      case WorkStage.mature:
        return 'Bonsai maturo';
    }
  }
}
